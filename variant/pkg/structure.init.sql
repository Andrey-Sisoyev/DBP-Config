...
CREATE TABLE configurable_entities (
        confentity_code_id       integer NOT NULL PRIMARY KEY USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
...
CREATE TABLE configurations (
      ...
      , PRIMARY KEY (confentity_code_id, configuration_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>;
...
CREATE INDEX confentities_configs_idx ON configurations(confentity_code_id) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs;
...
CREATE TABLE configurations_bylngs (
        confentity_code_id  integer NOT NULL REFERENCES configurable_entities(confentity_code_id) ON DELETE RESTRICT ON UPDATE CASCADE
      , configuration_id    varchar NOT NULL
      ...
      , completeness_as_regulator
                            t_completeness_as_regulator NULL
      , PRIMARY KEY (confentity_code_id, configuration_id, values_lng_code_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , FOREIGN KEY (values_lng_code_id)
...
COMMENT ON TABLE configurations_bylngs IS '
Rules for fields "completeness_as_regulator" and "complete_isit" are the same, as ones for table "configurations"
';

GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE configurations_bylngs TO user_db<<$db_name$>>_app<<$app_name$>>_data_admin;
GRANT SELECT                         ON TABLE configurations_bylngs TO user_db<<$db_name$>>_app<<$app_name$>>_data_reader;

CREATE INDEX confentities_configs_oflng_idx ON configurations_bylngs(confentity_code_id, configuration_id) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs;
...
CREATE TABLE configurations_names (
      ...
      , configuration_id   varchar NOT NULL
      , PRIMARY KEY (confentity_code_id, configuration_id, lng_of_name) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , FOREIGN KEY (confentity_code_id, configuration_id) REFERENCES configurations(confentity_code_id, configuration_id)
                                                            ON DELETE CASCADE  ON UPDATE CASCADE
      , FOREIGN KEY (lng_of_name) REFERENCES languages(code_id) ON DELETE RESTRICT ON UPDATE CASCADE
...
(1) Constraint function is a function that accepts $1 as a value of parameter :: varchar (subconfiguration ID in case, when parameter type is "subconfig")
(2) Constraint function is a function that accepts $2 as an ID of configuration :: varchar, for which the parameter value is specified
(3) Constraint function is a function that accepts $3 as an ID of language code :: integer, in which value is taken (it may be NULL)
(4) Constraint function return-type is boolean.
(5) If constraint function returns NULL it is treated as a constraint violation.
...
** mk_confparam_constraint(''SELECT my_constraint_function_that_returns_boolean($1,$2,$3)'')
...
Notice, that forst 2 parameters are of type varchar, but there is no need to put quotes around them inside a constraint function.
...
CREATE TABLE configurations_parameters (
       ...
       , PRIMARY KEY (confentity_code_id, parameter_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
       , UNIQUE (confentity_code_id, parameter_id, parameter_type) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
...
CREATE TABLE configurations_parameters_names (
      ...
      , PRIMARY KEY (confentity_code_id, parameter_id, lng_of_name) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
...
CREATE TYPE t_lnged_paramvalue_dflt_src AS ENUM ('param_dflt', 'nonlnged_val', 'pdflt_w_nonlng_null', 'null');

COMMENT ON TYPE t_lnged_paramvalue_dflt_src IS '
Types of sources for the default languaged values. Used only when according entry is not found in the table "configurations_parameters_lngvalues__leafs" or when value there is NULL.
Possible values:
 ** "paramval_dflt" - read parameter default as value
 ** "nonlnged_val"  - read nonlanguaged parameter value (from table "configurations_parameters_values__leafs")
 ** "pdflt_w_nonlng_null" - read nonlanguaged parameter value (from table "configurations_parameters_values__leafs")
                            , if it is NULL, read parameter default as value
 ** "null"          - leave value to be NULL
This applies only to languaged leaf-values and is used before taking "configurations_parameters"."use_default_instead_of_null" into consideration
';
...
CREATE TABLE configurations_parameters__leafs (
      ...
      , lnged_paramvalue_dflt_src t_lnged_paramvalue_dflt_src NOT NULL DEFAULT 'param_dflt'
      , PRIMARY KEY (confentity_code_id, parameter_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , UNIQUE (confentity_code_id, parameter_id, parameter_type) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
...
COMMENT ON TABLE configurations_parameters__leafs IS '
Field "lnged_paramvalue_dflt_src" field applies only for languaged values. It reads as "Type of source for the default languaged values". Used only when according entry is not found in the table "configurations_parameters_lngvalues__leafs" or when value there is NULL.
It is used before taking "configurations_parameters"."use_default_instead_of_null" into consideration.
';
...
CREATE TABLE configurations_parameters_values__leafs (
      ...
      , configuration_id   varchar NOT NULL
      , value              varchar     NULL
      , PRIMARY KEY (confentity_code_id, parameter_id, configuration_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , FOREIGN KEY (confentity_code_id, parameter_id)
...
CREATE INDEX pvl_configurations_of_confentities_idx
        ON configurations_parameters_values__leafs(confentity_code_id, configuration_id) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs;

-------------------------------------------------------------------------------
-------------------------------------------------------------------------------

CREATE TABLE configurations_parameters_lngvalues__leafs (
        confentity_code_id integer NOT NULL
      , parameter_id       varchar NOT NULL
      , configuration_id   varchar NOT NULL
      , value_lng_code_id  integer NOT NULL
      , value              varchar     NULL
      , PRIMARY KEY (confentity_code_id, parameter_id, configuration_id, value_lng_code_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , FOREIGN KEY (confentity_code_id, parameter_id, configuration_id)
                    REFERENCES configurations_parameters_values__leafs(confentity_code_id, parameter_id, configuration_id)
                    ON DELETE CASCADE  ON UPDATE CASCADE
      , FOREIGN KEY (confentity_code_id, configuration_id, value_lng_code_id)
                    REFERENCES configurations_bylngs(confentity_code_id, configuration_id, values_lng_code_id)
                    ON DELETE CASCADE  ON UPDATE CASCADE
) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>;

GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE configurations_parameters_lngvalues__leafs TO user_db<<$db_name$>>_app<<$app_name$>>_data_admin;
GRANT SELECT                         ON TABLE configurations_parameters_lngvalues__leafs TO user_db<<$db_name$>>_app<<$app_name$>>_data_reader;

CREATE INDEX plvl_configurations_of_confentities_idx
        ON configurations_parameters_lngvalues__leafs(confentity_code_id, configuration_id) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs;
CREATE INDEX plvl_configurations_of_confentities_bylng_idx
        ON configurations_parameters_lngvalues__leafs(confentity_code_id, configuration_id, value_lng_code_id) TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs;

COMMENT ON TABLE configurations_parameters_lngvalues__leafs IS
'If value for some language is not persistent in this table, then value is taken from "configurations_parameters_values__leafs" (not default value from "configurations_parameters__leafs"!).
This rule is used for completeness check of config in "configurations_bylngs".';
...
CREATE TABLE configurations_parameters__subconfigs (
      ...
      , PRIMARY KEY (confentity_code_id, parameter_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , UNIQUE (confentity_code_id, parameter_id, parameter_type)        USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , UNIQUE (confentity_code_id, parameter_id, subconfentity_code_id) USING INDEX TABLESPACE tabsp_<<$db_name$>>_<<$app_name$>>_idxs
      , FOREIGN KEY (confentity_code_id, parameter_id)
                    REFERENCES configurations_parameters(confentity_code_id, parameter_id)
                    ON DELETE CASCADE  ON UPDATE CASCADE
      , FOREIGN KEY (confentity_code_id, parameter_id, parameter_type)
